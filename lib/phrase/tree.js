...
exports.createClass = function(root) {
  var ChangeSet, PhraseTree, assembler, context, notice, trees;
  ...
  notice.use(assembler = function(msg, next) {
    ...
    switch (msg.context.title) {
      ...
      case 'phrase::edge:create':
        return trees.latest.registerEdge(msg, next);
      case 'phrase::leaf:create':
        return trees.latest.registerLeaf(msg, next);
      case 'phrase::recurse:end':
        return trees.latest.createIndexes(msg, next);
      default:
        return next();
    }
  });
  PhraseTree = (function() {
    ...
    PhraseTree.prototype.createIndexes = function(msg, next) {
      var recurse,
        _this = this;
    ...
    PhraseTree.prototype.update = function() {
      return pipeline([
        function() {
          return notice.event('tree::compare:start');
        }, function() {
          return new ChangeSet(context.tree, context.trees.latest).changes;
        }, function(changes) {
          return notice.event('tree::compare:end', {
            changes: changes
          });
        }, function(message) {
          return notice.event('tree::update:start', message);
        }, function(message) {
          ...
          return notice.event('tree::update:end', updated);
        }
      ]);
    };

    PhraseTree.prototype.registerEdge = function(msg, next) {
